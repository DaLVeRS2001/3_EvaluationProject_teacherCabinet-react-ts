...
import {ITBodyMProps, ITBodyProps, TParsedStudent} from "../../../types/props";


//const currentDay = weekDays.findIndex(el=> el === day)+1


export const TBodyM: React.FC<ITBodyMProps> = ({parseStudent, isMobile, countElHeight, randomColors}) => {
                ...
                if((availableIndex && isMobile.is) || (!isMobile)){
                    return <td  key={idx2} height={70}>
                        {isStudent
                            ? <div
                                style={{
                                    height: countElHeight(parsed[3], parsed[4]), background: getRandomColor(randomColors)
                                }}
